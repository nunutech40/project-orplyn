# Remaining Questions For Owner

Status 18 Juli 2026: Form Owner Tahap 1, Form Owner Tahap 2, dan Survey Customer awal sudah diisi. Jawaban terbaru sudah dipindahkan ke `business-data/owner-answers.md`. Jangan mengulang pertanyaan yang sudah terjawab kecuali butuh approval publik.

## P0 - Sebelum Public Launch

- Apakah owner menyetujui nomor WhatsApp publik `082317579311` / `6282317579311` untuk semua CTA website?
- Target maksimal balas lead baru: kurang dari 5 menit, 5-15 menit, 16-30 menit, 31-60 menit, atau lebih?
- Format quotation final: cukup chat WhatsApp, perlu PDF, spreadsheet, aplikasi, atau kombinasi?
- Apakah owner menyetujui wording publik untuk MOQ dan lead time per produk?
- Apakah owner menyetujui wording publik untuk kaos polos mulai Rp33.000/pcs?
- Apakah owner menyetujui wording publik untuk rush order, revisi desain, DP, refund, dan QC/rework?
- Review, testimoni chat, nama/logo customer, dan case study mana yang sudah punya izin publikasi?
- Foto workshop, proses produksi, packing, hasil jadi, tim, atau owner video mana yang boleh dipakai?

## P1 - Sebelum Ads Diperbesar

- Tier harga grosir detail per produk: 6-11, 12-23, 24-49, 50-99, 100+ pcs, atau struktur lain?
- Produk atau layanan apa yang kapasitasnya paling aman untuk didorong 30 hari ke depan?
- Produk apa yang sedang overload atau sebaiknya tidak diiklankan?
- Teknik mana yang dikerjakan sendiri dan mana yang melalui partner?
- Kapasitas produksi aman per minggu/bulan untuk kaos polos, DTF, sablon manual, bordir, jersey, fullprint, dan totebag?
- Omzet rata-rata 6-12 bulan terakhir?
- Jumlah order rata-rata per bulan?
- Average order value keseluruhan, satuan, dan grosir?
- Komposisi omzet satuan vs grosir?
- Margin atau gross profit range per produk utama?
- CAC atau biaya iklan per customer yang masih dianggap sehat?

## P2 - Untuk Optimasi

- Data lead/order 90 hari bisa diekspor atau direkap manual?
- Musim ramai/sepi sebenarnya kapan jika dilihat dari order lama?
- Kompetitor yang paling sering dibandingkan pelanggan siapa saja?
- Objection mana yang paling sering membuat lead batal setelah diberi harga?
- Apakah ada katalog bahan/warna/pricelist yang bisa dibuat owner/admin dari data lama?
- Customer mana lagi yang bisa diwawancarai agar total 8-12 responden?

## Supporting Discovery Files

- `owner-discovery-survey.md`: master survey owner, P0 sampai P2.
- `customer-discovery-survey.md`: survey customer menang dan lost leads.
- `customer-data-template.md`: struktur data lead/order 90 hari dan rekap 12 bulan.
- `business-data/owner-answers.md`: jawaban owner/admin terbaru.
- `business-data/findings-register.md`: fakta, observasi, hipotesis, gap, dan keputusan.
