...
Latest public release remains `orplyn-web:20260722T054152Z-afe36fd` and is healthy. A newer D-016 brand-character candidate is running locally for owner review; it has not been committed, pushed, or deployed. Google Ads remains OFF and indexing remains disabled until the remaining launch gates are explicitly cleared.
...
1. Review the local D-016 homepage, portfolio, contact, and event LP on a real phone; approve or revise the Legoso/Aulia/process/contextual-work narrative before any commit or deployment. Then run the end-to-end WhatsApp CTA test with Aulia.
2. Lint, production build, 9 rendered smoke tests, local container health, and four target-route checks are complete for the D-016 candidate. Run responsive screenshot/manual QA when browser tooling is available; public verification applies only after an approved deployment.
